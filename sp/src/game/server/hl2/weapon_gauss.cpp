...
void CWeaponGauss::ChargedFire(void)
{
	...
	if (pHit != NULL)
	{
		CTakeDamageInfo dmgInfo(this, pOwner, flDamage, DMG_ENERGYBEAM); // DMG_CHARGEDGAUSS
		CalculateBulletDamageForce(&dmgInfo, m_iPrimaryAmmoType, aimDir, tr.endpos);
...
//	int pitch = 100; //(gpGlobals->curtime - m_flChargeStartTime) * (200 / GetFullChargeTime()) + 100;
