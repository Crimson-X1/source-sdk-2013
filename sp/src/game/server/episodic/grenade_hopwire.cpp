...
#ifdef CRIMSON_MOD
ConVar hopwire_aftershock_enabled("hopwire_aftershock_enabled", "1", FCVAR_CHEAT, "If enabled, vortex grenades will produce an aftershock (\"concussive_blast\") when it stops sucking things in." );

// Mass requirement for the aftershock (Should this be changed to a skill variable?)
ConVar hopwire_aftershock_mass("hopwire_aftershock_mass", "250", FCVAR_CHEAT, "Sets the minimum mass the vortex requires to produce an aftershock when it collapses.");
#endif
...
{
	pEntity->EmitSound("WeaponVortexGrenade.SpawnXenPC");
...
bool CGravityVortexController::TryCreateComplexNPC(const char *className, bool isBaby, bool isBird) {
	...
	EmitSound("WeaponVortexGrenade.SpawnXenPC");
...
void CGravityVortexController::SchlorpThink(void)
{
	if (m_flSchlorpMass > 0.0f)
	{
		...
		if (m_flSchlorpMass >= hopwire_schlorp_huge_mass.GetFloat())
			pszSoundName = "WeaponVortexGrenade.Schlorp_Huge";
		else if (m_flSchlorpMass >= hopwire_schlorp_large_mass.GetFloat())
			pszSoundName = "WeaponVortexGrenade.Schlorp_Large";
		else if (m_flSchlorpMass >= hopwire_schlorp_medium_mass.GetFloat())
			pszSoundName = "WeaponVortexGrenade.Schlorp_Medium";
		else if (m_flSchlorpMass >= hopwire_schlorp_small_mass.GetFloat())
			pszSoundName = "WeaponVortexGrenade.Schlorp_Small";
		else
			pszSoundName = "WeaponVortexGrenade.Schlorp_Tiny"; // Tiny (default)
...
void CGravityVortexController::PullThink( void )
{
	...
	for ( int i = 0; i < numEnts; i++ )
	{
		...
		{
			DevMsg( "Vortex grenade turned NPC '%s' into a ragdoll! \n", pEnts[i]->GetDebugName() );
		}
		else
		{
...
void CGrenadeHopwire::Precache( void )
{
	...
	PrecacheScriptSound( "WeaponVortexGrenade.Aftershock");
//	PrecacheScriptSound( "WeaponVortexGrenade.SpawnXenPC" );
	PrecacheScriptSound( "WeaponVortexGrenade.Blip" );
	PrecacheScriptSound( "WeaponVortexGrenade.Hop" );

	PrecacheScriptSound( "WeaponVortexGrenade.Schlorp_Huge" );
	PrecacheScriptSound( "WeaponVortexGrenade.Schlorp_Large" );
	PrecacheScriptSound( "WeaponVortexGrenade.Schlorp_Medium" );
	PrecacheScriptSound( "WeaponVortexGrenade.Schlorp_Small" );
	PrecacheScriptSound( "WeaponVortexGrenade.Schlorp_Tiny" );

	// New particle based explosion
	PrecacheParticleSystem( "ExplosionVortex" );
//	PrecacheParticleSystem("ExplosionCore");
//	PrecacheParticleSystem("Explosion_Vortex_Warp");
...
void CGrenadeHopwire::DelayThink()
{
	if( gpGlobals->curtime > m_flDetonateTime )
	{
		if (m_pMainGlow)
		{
			m_pMainGlow->SetTransparency( kRenderTransAdd, 0, 60, 255, 255, kRenderFxNoDissipation );
...
void CGrenadeHopwire::CombatThink( void )
{
	...
	// FIXME: Replace
	EmitSound("WeaponVortexGrenade.Explode"); // Sound to emit during detonation
...
void CGrenadeHopwire::Detonate( void )
{
	...
	//EmitSound("WeaponVortexGrenade.Explode");
	SetModel( szWorldModelOpen );

	EmitSound( "WeaponVortexGrenade.Hop" );
...
CBaseGrenade *HopWire_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, const char * modelClosed, const char * modelOpen)
{
	...
	static const char *szHopwireModel = "models/weapons/w_vortexgrenade.mdl";
	if (modelClosed == NULL)
